...
export async function updateRecord(tableName, recordUuid, payload) {
  const keys = Object.keys(payload);
  if (keys.length === 0) {
    const q0 = `SELECT * FROM "${tableName}" WHERE record_uuid = $1;`;
    const r0 = await pool.query(q0, [recordUuid]);
  ...
  const q = `UPDATE "${tableName}" SET ${setClause} WHERE record_uuid = $${values.length} RETURNING *;`;
  const r = await pool.query(q, values);
